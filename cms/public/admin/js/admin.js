/**
 * MindFull Command Center v4.0
 * Event-Delegation Architecture (Industrial Strength)
 */
...
(function () {
  ...
  console.log('MindFull Admin v4.0: Event Delegation Active');
  ...
  document.addEventListener('click', async (e) => {
    ...
    console.log(`Action Triggered: ${action}`, { id, slug });
    ...
    switch (action) {
      ...
      case 'logout':
        ...
        break;
      case 'edit-entry':
        editEntry(btn.getAttribute('data-collection'), slug);
        break;
      case 'delete-entry':
      ...
      case 'view-profile':
        ...
        break;
      case 'open-reply':
        openReplyModal(id, email, name);
        break;
      case 'use-template':
        useTemplate(btn.getAttribute('data-subject'), btn.getAttribute('data-body'), name);
        break;
      case 'send-reply':
        sendQuickReply();
        break;
      ...
      case 'load-collection':
        loadCollection(btn.getAttribute('data-name'));
        break;
    }
  });
    ...
    if (popular) {
      document.getElementById('popular-list').innerHTML = popular.map(i => `
        <div class="flex items-center justify-between py-3 px-4 bg-slate-50 rounded-xl mb-2">
          ...
          <span class="text-[10px] font-black text-indigo-600">${i.views} VIEWS</span>
        </div>
      `).join('');
    }
  }
    ...
    container.innerHTML = items.map(i => `
      <div class="admin-card p-6 flex flex-col h-full group">
        ...
        <div class="flex gap-2">
          <button data-action="edit-entry" data-collection="${name}" data-slug="${i.slug}" class="flex-1 py-2 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-lg shadow-lg">Edit</button>
  ...
  async function loadUsers() {
    ...
    container.innerHTML = users.map(u => `
      <tr class="hover:bg-slate-50 transition-colors">
        ...
        <td class="px-8 py-6 text-right"><button data-action="view-profile" data-id="${u.id}" class="text-indigo-600 font-black text-[10px] uppercase">Profile &rarr;</button></td>
      </tr>
    `).join('');
  }
  ...
  async function viewUserProfile(id) {
    ...
    document.getElementById('user-profile-info').innerHTML = `
      <div class="flex items-center gap-4 mb-8">
        <div class="w-16 h-16 bg-indigo-600 rounded-2xl flex items-center justify-center text-white text-2xl font-black">${profile.user.name[0]}</div>
        <div><h3 class="text-xl font-black text-slate-900">${profile.user.name}</h3><p class="text-xs font-bold text-indigo-600 uppercase">${profile.user.role}</p></div>
      </div>
      <div class="space-y-4 pt-6 border-t border-slate-100">
        <div><p class="text-[10px] font-black text-slate-400">EMAIL</p><p class="text-sm font-bold text-slate-700">${profile.user.email}</p></div>
      </div>
    `;
    ...
    `).join('') || '<p class="py-10 text-center text-xs italic text-slate-400">No activity.</p>';
  }
  ...
  async function loadInquiries() {
    ...
    container.innerHTML = leads.map(l => `
      <tr class="${l.status === 'new' ? 'bg-indigo-50/20' : ''} hover:bg-slate-50 transition-colors">
        ...
        <td class="px-8 py-6 text-xs text-slate-500 line-clamp-2">${l.message}</td>
        <td class="px-8 py-6 text-right"><button data-action="open-reply" data-id="${l.id}" data-email="${l.email}" data-name="${l.name}" class="bg-white border border-slate-200 text-indigo-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase">Quick Reply</button></td>
      </tr>
    `).join('');
  }
  ...
  function useTemplate(subject, escapedBody, name) {
    const body = decodeURIComponent(escapedBody).replace('{{name}}', name);
    ...
    container.innerHTML = items.map(i => `
      <div class="admin-card p-2 relative group overflow-hidden">
        ...
        <div class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-opacity gap-2">
          <button data-action="copy-url" data-url="${i.url}" class="p-2 bg-indigo-600 text-white rounded-lg text-[8px] font-bold uppercase tracking-widest px-3">Copy URL</button>
          <button data-action="delete-media" data-name="${i.name}" class="p-2 bg-red-500 text-white rounded-lg text-[8px] font-bold uppercase tracking-widest px-3">Delete</button>
        </div>
      </div>
    `).join('');
  }
