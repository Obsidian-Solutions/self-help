...
(function () {
  ...
  async function viewUserProfile(id) {
    ...
    document.getElementById('user-profile-info').innerHTML = `
      ...
      <div class="space-y-4 pt-6 border-t border-slate-100">
        <div><p class="text-[10px] font-black text-slate-400">IDENTITY</p><p class="text-sm font-bold text-slate-700">${profile.user.email}</p></div>
      </div>
    `;
    ...
    `).join('') || '<p class="py-10 text-center text-xs italic text-slate-400">No telemetry data.</p>';
  }
  ...
  async function loadMedia() {
    ...
    if (!items || items.length === 0) {
      container.innerHTML = '<p class="col-span-full py-20 text-center text-xs italic text-slate-400">Resource library empty.</p>';
      return;
    }
    container.innerHTML = items.map(i => `
